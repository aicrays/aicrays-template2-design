      ...
      <div className="section-container relative z-10">
        ...
        <div className="hidden lg:block relative mb-12">
          <div className="absolute top-8 left-[12.5%] right-[12.5%] h-0.5 bg-gray-200 rounded-full">
            <div 
              className="h-full bg-gradient-to-r from-pulse-400 to-pulse-600 rounded-full transition-all duration-1000 ease-out"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {processSteps.map((step, index) => (
            <div 
              key={step.title}
              className={`process-step text-center group cursor-pointer transition-all duration-500 ${
              ...
              <div className="relative mb-6">
                ...
                {index < processSteps.length - 1 && (
                  <div className="hidden lg:block absolute top-8 left-full w-full h-0.5 -translate-x-8">
                    <div className={`h-full bg-gradient-to-r from-gray-300 to-transparent transition-all duration-500 ${
                      visibleSteps.includes(index + 1) ? 'from-pulse-400 to-pulse-200' : ''
                    }`}></div>
                  </div>
                )}
